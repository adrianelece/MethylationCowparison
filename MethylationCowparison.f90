...
!Assign cols
    ...
    do i=1,nlinesONT
	    read (10,'(6i12,f4.10)') chrONT(i),startONT(i),endONT(i),col1(i),covONT(i),col3(i),freqONT(i)
    end do

    do i=1,nlinesBS
	    read (20,'(4i12,f4.10)') chrBS(i),startBS(i),covBSunmeth(i),covBSmeth(i),freqBS(i)
    end do
...
write (*,*) "|__________________________________________________________________________|"
     ...
     if (i.gt.nlinesONT) then
        write(*,*) "Finalizado archivo de ONT"
        goto 15
     end if  
     if	(j.gt.nlinesBS) then
        write(*,*) "Finalizado archivo de BS con j=", j
        goto 15
     end if

if (chrBS(j)==chrONT(i)) then
    if (startONTcorrected(i)<=startBS(j) .and. endONTcorrected(i)>=startBS(j)) then
        write(30,'(i4,i10,2f12.6,2i4)') chrBS(j),startBS(j), freqBScorrected(j),freqONT(i),covBS(j),covONT(i)
        j=j+1
    else if (startONTcorrected(i)>startBS(j)) then
        j=j+1
        write (*,*) "Mismo chr, primer else if"
    else if (endONTcorrected(i)<startBS(j)) then
        i=i+1
        write (*,*) "Mismo chr, segundo elseif"
    end if    
else if (chrBS(j)<chrONT(i)) then
    j=j+1
    write (*,*) "Primer if de chr. Chr ONT: ", chrONT(i), " ChrBS: ", chrBS(j)
else if (chrBS(j)>chrONT(i)) then
    write (*,*) "Segundo if de chr. Chr ONT: ", chrONT(i), " ChrBS: ", chrBS(j)
    i=i+1
end if 
